function [xY, XYZmm, j] = spm_ROI_tim(xY, XYZmm, ROI_identifyer)
    ...
    % ROI_identifyer: if ROI specified as mask, then all voxels in image with
    %           number corresponding to ROI_identifyer are selected (TR)
    %
    ...
    switch lower(xY.def)
        ...
        case 'mask'
            ...
            j      = find(spm_sample_vol(xY.spec, XYZ(1, :), XYZ(2, :), XYZ(3, :), 0) == ROI_identifyer);
    ...
    XYZmm      = XYZmm(:, j);
    if strcmpi(xY.def, 'mask') && ~isempty(XYZmm)
        xY.xyz = mean(XYZmm, 2);
    end
