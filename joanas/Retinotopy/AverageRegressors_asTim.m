...
for subj = subjects_list
    ...
    for run = 1:no_of_runs
        columns_sess{run} =  SPM.Sess(run).col;
    ...
    for ind = length(Sine_hdr)
        if strcmp(Sine_hdr(ind).descrip(end - 12:end), 'Minus^1*bf(1)')
            Sine_vols(:, :, :, ind) = -spm_read_vols(Sine_hdr(ind)); %#ok<*SAGROW>
        else
            Sine_vols(:, :, :, ind) = spm_read_vols(Sine_hdr(ind));
        end
    end
    ...
    meanSineBetas_hdr.fname = sprintf('%s\\AsTim_PolarRetino_subj%d_meanSineBetas.img', RETINO_folder, subj);
    ...
    meanCosineBetas_hdr.fname = sprintf('%s\\AsTim_PolarRetino_subj%d_meanCosineBetas.img', RETINO_folder, subj);
    ...
    phase_hdr.fname = sprintf('%s\\AsTim_PolarRetino_subj%d_phase.img', RETINO_folder, subj);
    ...
    amplitude_hdr.fname = sprintf('%s\\AsTim_PolarRetino_subj%d_amplitude.img', RETINO_folder, subj);
